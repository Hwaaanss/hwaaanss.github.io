...
- **FPGA 최적화 미흡**: FPGA 최적화가 미비하여 TMATMUL 연산 시간이 지나치게 길다는 점은 개선이 필요하다. FPGA 최적화를 해내면 더욱 좋은 성능을 낼 수 있을 것으로 생각된다.

### 시사점 및 추가 연구 아이디어
본 연구는 언어 모델 설계에서 MatMul이 꼭 필요하지 않다는 것을 증명하여, 향후 LLM에서 연산 최적화를 위한 새로운 연구 방향성을 제시한다. 추가 연구로는 초대형 모델에 대한 MatMul-free 아키텍처 검증과, 다양한 하드웨어에서의 최적화 방안이 유망하다. 또한 이러한 기술들이 많이 발전되면 현재 엔비디아 H100과 같은 초고성능의 칩을 향한 시장의 쏠림 또한 어느정도 완화될 가능성을 보인다. 추가적으로 이러한 기술들의 발전을 통해 학습을 돌리기에 자원이 부족한 학생들이나 개인이 유의미한 효과를 볼 것으로 생각된다.  
