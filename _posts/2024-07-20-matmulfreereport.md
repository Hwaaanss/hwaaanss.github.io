---
layout: single
title: "논문 리뷰: Scalable MatMul-free Language Modeling"
...
## 3. 이 연구를 진행하는 데 필요한 배경지식

### 유사한 transformer 대체 연구
- **Mamba**: SSM(State Space Model)을 뼈대 아키텍쳐로 갖고 있다. 거대 데이터를 활용한 시퀀스 모델링에 탁월하다. 
- **TTT(Test Time Training)**: 스탠포드대학교, UC 샌디에이고, UC 버클리, 메타의 연구진이 발표한 아키텍처로 transformer의 attention 메커니즘의 히든 스테이트를 머신러닝 모델로 대체한 방식이다.  

### Key-word
- **Ternary Weight**: 메모리와 계산 효율을 위해 개발된 3항 가중치 구조. 가중치는 -1, 0, 1로 제한된다.
- **RMSNorm**: 신경망의 가중치를 정규화하는 L1/L2 Regularization과 다르게 신경망의 입력을 정규화하는 방법이다. 각 특성의 평균을 고려하지 않고, 입력의 제곱평균을 사용하여 정규화를 수행한다. 경량화된 계산을 필요로 하는 모델에서 주로 사용된다.
- **Zero-shot**: 일반적으로 딥러닝의 경우 학습에 사용된 데이터와 class만을 예측할 수 있는데, ZSL(Zero-Shot Learning)은 테스트 시 학습에 사용되지 않은(처음보는) 데이터를 보고 학습에 사용되지 않은(처음보는) class를 예측한다. 이에 더 나아가서 GZSL(General Zero-Shot Learning) 이라는 것은 테스트 시 처음보는 데이터와 학습했던 데이터를 같이 입력받고, 같이 추론한다. 따라서 대부분의 상황에서 GZSL이 적합하다. 
...
$\text{3.}$ **GRU 기반 Token Mixer**: 토큰 믹싱(token mixing) 단계에서 MatMul을 배제하고, GRU의 element-wise 연산을 통해 정보 통합을 수행했다.
  ...
  - **GRU 기반 Token Mixer**
    GRU(Gated Recurrent Unit)는 RNN의 변형으로, LSTM(Long Short-Term Memory)보다 단순하면서 유사한 성능을 제공하는 구조이다. 본 논문에서는 GRU를 기반으로 곱셈 연산을 배제하고 요소별 연산과 누적 덧셈으로 구성된 **MatMul-Free GRU(MLGRU)**를 설계하였다.
...
#### 고찰 및 추가 설명
MatMul-Free GRU는 전통적인 GRU와 비교해 단순화된 구조로, 하드웨어에서 효율적으로 실행될 수 있도록 설계되었다. 곱셈을 제거함으로써 메모리 대역폭과 전력 소모를 줄이는 데 큰 장점이 있으며, 삼진 가중치를 사용해 학습의 복잡성을 줄이면서도 성능 저하를 최소화할 수 있다. 이러한 설계는 대규모 모델을 저전력 환경에서 실행할 때 매우 유리하며, FPGA(Field-Programmable Gate Array)나 ASIC(Application-Specific Intergrated Circiut) 같은 특수 하드웨어 가속기에서 높은 효율성을 발휘할 수 있다.
...
- **자원 효율성**: GPU에서 MatMul-free 모델의 메모리 사용량은 기존 모델 대비 최대 61% 절감되었고, 연산 속도는 4.57배 증가하였다. 이는 메모리 대역폭이 제한된 환경에서도 효율적으로 작동할 수 있는 가능성을 보여주며, 데이터 이동 및 병렬 처리를 최적화한 결과이다. MatMul-free 모델의 가벼운 연산은 특히 대규모 데이터센터뿐만 아니라 저전력 디바이스에서도 활용될 수 있는 장점을 제공한다. 

### 결과의 의미와 해석
대규모 언어 모델에서도 필수적인 것으로 여겨지던 MatMul 연산이 불필요하다는 것을 실험적으로 증명하였다. 이러한 발견은 머신러닝 연구 및 산업에서 새로운 방향성을 제시할 수 있다. 특히, 학생이나 개인 혹은 작은 스타트업과 같이 학습을 위한 자원이 제한된 환경에서 언어 모델의 활용성을 높이고, 더 나은 에너지 효율성을 요구하는 애플리케이션에서도 적용할 수 있는 기반을 마련하였다.

**고찰**: MatMul-free 모델은 삼진 가중치와 같은 간단한 연산으로도 복잡한 패턴을 학습할 수 있음을 보여주었다. 이는 하드웨어 설계 측면에서도 의미가 크며, 향후 FPGA, ASIC과 같은 전용 하드웨어에서 MatMul-free 모델의 채택 가능성을 높인다. 이러한 혁신은 머신러닝 모델의 효율성을 극대화하여 연구 및 상업적 활용에서 큰 영향을 미칠 수 있다.
...
- **연산 최적화 부족**: 현재 FPGA에서 TMATMUL(Ternary Matrix Multiplication) 기능 유닛이 전체 처리 시간의 99%를 차지하는 등 최적화가 미비한 부분이 존재한다.
...
### 연구의 전체적인 요약 및 결론
본 연구는 대형 언어 모델에서 많은 메모리를 사용하는 MatMul 연산을 제거하고, 대체 연산을 통해 자원 절약과 성능을 동시에 달성할 수 있음을 증명하였다. 특히, 대형 모델이 GPU, FPGA 등의 하드웨어에서 더 높은 전력 효율을 보이게 하여, 차세대 LLM의 경량화와 상용화 가능성을 제시하였다.
...
- **실험적 검증**: 다양한 실험을 통해 모델의 확장성과 자원 효율성을 성공적으로 입증했다.

### 논문의 약점
- **초대형 모델 적용**: 연구가 2.7B 파라미터로 제한되어 파라미터의 크기가 더욱 많은 초대형 LLM에서의 성능 확인이 필요하다.
- **FPGA 최적화 미흡**: FPGA 최적화가 미비하여 TMATMUL 연산 시간이 지나치게 길다는 점은 개선이 필요하다. FPGA 최적화를 해내면 더욱 좋은 성능을 낼 수 있을 것으로 보여진다.
...
위의 Compute-Loss 그래프를 보면 학습 연산량이 적은 상황에서 Transformer++ 보다 더 나은 성능을 보여준다. 이는 sLLM 모델에 적합하다고 보여져 모바일 시장에서 활용될 가능성도 있고, 학습을 돌리기에 자원이 부족한 학생들이나 개인이 유의미한 효과를 볼 것으로 보여진다. 

추가 연구로는 초대형 모델에 대한 MatMul-free 아키텍처 검증과, 다양한 하드웨어에서의 최적화 방안이 유망하다. 
