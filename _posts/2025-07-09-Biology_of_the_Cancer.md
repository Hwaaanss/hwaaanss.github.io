...
#### 1. 암종(Carcinoma)
여러 기관을 이루는 epithelial cell이 변형되어 생기는 암이다. `squamous epithelial tissue`에서 발생한 암을 의미한다. 예를 들어 유방암, 췌장암 등이 속한다. 
...
#### 2. 육종(Sarcoma)
`Mesenchymal tissue`로 부터 유래한 암이다. 뼈, 연골, 지방 등 조직에 생긴 암을 의미한다.
...
#### 4. 신경외배엽(neuroectodermal tissue)의 종양
신경세포에 생기는 암 및 피부의 `melanoma`이 속한다.
- `melanoma`(흑색종) : 색소를 생성하는 피부 세포(멜라닌 세포)에서 시작되는 피부암이다.
...
암은 복잡한 과정을 거쳐 발생하며, 주로 암 개시과정, 진행과정, 전이과정 이렇게 세 단계로 구분된다.
...
### 3. 암 전이과정(Metastasis)
암의 전이과정은 크게 암 발생 - `Local Invasion` - Spead - `Metastasis` 로 볼 수 있다.
- `Local invasion`(국소침윤) : 주변 조직(Surrounding tissue)을 파괴(destruction)하면서 침윤(infiltration)하는 것이다. 이는 악성 종양을 판단하는 데에 있어서 가장 신뢰할만한 특징 중 하나이다. 이러한 소견이 보이면 환자는 수술적 제거와 항암치료를 동반하여서 진행하는 것이 가장 좋은 방법이다.
- `Metastasis`(전이) : 전이가 확인되면 암이 4기로 볼 수 있다. 전이는 수술적 제거를 하지 못하고, 항암치료나 국소 방사선치료를 하게 된다. 전이에는 `직접 파종성 전이`, `림프성 전이`, `혈행성 전이` 이렇게 세가지가 있다.
- `직접 파종성 전이`(direct seeding of body cavities or surfaces) : 악성종양의 local invasion이 심해져서 복강이나 흉강 등 체강부위로 관통 후 암세포들이 체강(흉강, 복강, 심낭, 관절강 등) 내로 파종되어 발생하는 케이스를 말한다. 이 이후 복강 내를 떠돌던 암세포들이 다른 장기의 표면에 내려앉아 새로운 종양을 형성한다. 위암이나 난소암 등의 경우 복강 내로 직접 파종성 전이가 일어날 수 있다.
- `림프성 전이`(lymphatic spread) : 림프관을 통해서 전이되는 것을 말한다. 림프성 전이는 가장 흔한 전이 방법이다. 림프성 전이를 알아보는 방법으로는 regional LNs(부분 림프노드)를 꺼내어 암세포를 발견할 수도 있고, 림프노드 자체가 이미 커져 있어도 림프성 전이가 일어났다고 간접적으로 알 수 있다. 하지만 가장 좋은 방법은 PET CT를 찍는 것이다. PET CT를 찍으면 암은 검정색으로 보여 진단에 용이하다. 
- `혈행성 전이`(hematogenous spread) : 혈관을 통해서 전이되는 것을 말한다. 예를 들어 colon(대장)에서 영양분을 흡수하고 나면 그것이 혈관을 통해 liver(간)으로 간다. 즉, colon에서 생긴 암은 혈행성 전이에 의해 liver로 전이될 수 있음을 말한다. (강의 13:38~ 이어서 정리) 
